...
every other action kind Kasten exposes. Built the same way as
[cpouthier/malware-scan](https://github.com/cpouthier/malware-scan): a
small web UI, the same Veeam-green look, SMTP settings with a write-only
password Secret, and a portable Helm chart.
...
```
$ kubectl get backupactions -A --watch
...
Error from server (MethodNotAllowed): watch is not supported on resources of kind "backupactions.actions.kio.kasten.io"

$ kubectl get runactions -A --watch
...
Error from server (MethodNotAllowed): watch is not supported on resources of kind "runactions.actions.kio.kasten.io"
```

So there's no event-driven alternative - this app periodically lists every
selected action kind and diffs against what it's already seen (a small
SQLite table on the data PVC). The interval is configurable on the
Settings page (minimum 60s).
...
chain Kasten recorded on the object itself (`status.error`) - no separate
pod log lookup needed, it's already on the object.
...
RetireAction. The first five are selected by default; the rest (mostly
...
```bash
docker build -t <you>/kasten-alerting:latest .
docker push <you>/kasten-alerting:latest
